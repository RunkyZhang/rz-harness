...
```bash
source config/runtime_local.sh
...
2. 测试账号和密码只能来自本地 ignored 的 `config/runtime_local.sh`、Keychain 或已有浏览器保存密码 / 登录态；版本化文档不得记录明文密码。
...
```bash
npm install
scripts/frontend-lint-build.sh "$RZ_REPO_MAP_SYSTEM" lint-files <changed-files...>
npm run build:test
```
