...
| Frontend repo | `$SFA_REPO_FRONTEND_MAP_SYSTEM` |
| Backend repo | `$SFA_REPO_BACKEND_SALES_MANAGEMENT` |
...
- [ ] 测试账号、登录态或免登录条件已明确；如需要密码，来源只能是 `config/repos.local.sh`、Keychain 或已有浏览器登录态，不能写入本计划。
...
- [ ] 如目标是 `mapSystem`，已读取 `docs/baseline/frontend-map-system.md`，并优先使用 `scripts/frontend-dev-server.sh frontend-map-system 9527` 或记录等价的已验证 Node `14.21.3` 启动命令。
...
| Test account | 写入测试账号标识；如来自本地配置，写 `SFA_PC_E2E_USERNAME configured` |
...
如果只启动一个后端，`--active-backends` 只写一个 repo id；如果同时启动多个后端，用逗号分隔。各后端本地地址来自 `SFA_LOCAL_BACKEND_*_URL`。如果不使用本地代理，写明原因，例如本次没有后端改动、后端改动不被当前前端直接调用，或只做测试环境 smoke。
...
1. 打开 `Target URL`。
2. 如果跳转登录页，优先使用已有 Chrome 登录态；否则读取本地-only 的 `SFA_PC_E2E_USERNAME` 和密码来源，或记录为人工输入 / `BLOCKED`。不得把密码写入报告、截图说明或 evidence。
...
- 本地代理只在 `SFA_HARNESS_SMOKE=1` 的 harness smoke 中启用，不影响普通 `npm run dev`。
