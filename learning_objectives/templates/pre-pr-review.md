...
- [ ] `changes/<change-id>/technical-solution.md` 已人工确认，并通过 `scripts/technical-solution-gate.sh changes/<change-id>`。
- [ ] 多仓 / 全栈需求已通过 `scripts/workstream-dispatch-gate.sh changes/<change-id>`；如降级为单 Agent 串行，`harness-status.md` 已记录 `DEGRADED:` 原因。
- [ ] 业务仓首次改代码前已通过 `scripts/business-code-start-gate.sh changes/<change-id> <changed-files...>`；没有在 `main/master` 直接修改业务代码。
- [ ] 若业务仓存在接手前 dirty diff，已通过 `scripts/business-dirty-worktree-gate.sh <repo> --ledger changes/<change-id>/dirty-worktree-ledger.md`，每个 dirty path 有 owner 和 decision。
- [ ] 所有阻塞性 `[QUESTION]` 已解决。
- [ ] 没有把 `[ASSUMP]` 内容实现进代码。
- [ ] 范围没有超出已批准的仓库和路径。
...
- [ ] 已执行 `scripts/skill-usage-gate.sh changes/<change-id>`；没有遗留 TODO，N/A 行均有原因。
...
- [ ] 已执行 `scripts/diff-hygiene-gate.sh <repo-root> [--base <ref>] <changed-files...>`，确认没有无关空白 / 格式噪音进入 review。
...
- [ ] 如进入测试 / 预发发布，已执行 `scripts/ai-test-report-gate.sh changes/<change-id>`；报告为 `CONFIRMED` 且 `recommendation: 允许进入预发`。
...
- [ ] 如本次新增 active Swagger 2 profile 覆盖范围内的 public REST response `*VO.java`，已执行 `scripts/swagger-model-documentation-gate.sh changes/<change-id> <changed-files...>`；模型和字段说明均通过。
...
- [ ] 已执行 `scripts/temp-hardcode-scan.sh <changed-files...>` 扫描 `CODX` / `smoke` / `mock` / `localhost` / `token` / `password` / `TODO` / `FIXME`。
...
- [ ] 独立 Reviewer Agent 已按 `skills/reviewer/SKILL.md` 审查 spec、contract、technical-solution、diff、evidence、skill usage、Test Agent、AI test report 和 PC E2E Smoke 证据。
...
- [ ] 已执行 `scripts/reviewer-gate.sh changes/<change-id>`；`high_risk_count: 0`，所有必查面均为 `PASS`。
...
- [ ] Reviewer Agent 之后，用户已做最终人工 review。
