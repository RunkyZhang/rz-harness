...
| Frontend baseline | `docs/baseline/frontend-map-system.md` |
| Backend baseline | `docs/baseline/backend-sales-management.md` |

## Frontend Style References

```yaml
style_sample_references_status: PENDING
style_profile: docs/baseline/<frontend-repo>-style-profile.md
```
...
> 每个实施任务、实现 Agent、测试 Agent 和 Reviewer 都默认继承本节。只写已确认事实；
...
| `<本次允许修改的 repo / module / path>` | `contract.md allowed_paths` | `scripts/business-code-start-gate.sh changes/<change-id> <files...>` |
| `<字段、状态、权限、错误码、默认值等已确认事实>` | `spec.md` / PRD / 用户确认 | `verification-map.md` / targeted test / PC smoke |
| `<版本、运行环境、Node/Maven/profile 限制>` | `docs/baseline/*` / `environment-readiness.md` | `canonical-command-gate.sh` / repo-specific command |

## Task Handoff Contract

> 多 Agent 或长上下文执行时，Orchestrator 应把任务正文、报告和 review diff 写入
...
- [ ] 已运行 `scripts/technical-solution-gate.sh changes/<change-id>`；技术方案 `confirmation_status: CONFIRMED` 且 `allowed_next_stage` 允许进入当前阶段。
- [ ] 已复制 `templates/verification-map.md` 到 `changes/<change-id>/verification-map.md`，将关键约束映射到验证命令、人工确认或 `N/A:` 原因，并运行 `scripts/verification-map-gate.sh changes/<change-id>`。
...
- [ ] 需要派发 Codex/OpenCode 子 Agent 时，已运行 `scripts/agent-dispatch-plan.sh --stage <stage> --runtime <runtime> --change-id <change-id> --output changes/<change-id>/agent-dispatch-plan.md`，并运行 `scripts/agent-dispatch-plan-gate.sh changes/<change-id>`；candidate implementation agent 只有在 contract、allowed paths、隔离分支和 code-start gate 满足后，复制并确认 `templates/agent-candidate-confirmation.md`，再显式 `--allow-candidate --candidate-confirmation changes/<change-id>/agent-candidate-confirmation.md`。
- [ ] 业务仓首次改代码前已运行 `scripts/business-code-start-gate.sh changes/<change-id> <changed-files...>`；业务仓不在 `main/master`，且文件在 `contract.md` 的 `allowed_paths` 内。
- [ ] 如业务仓已有 dirty diff，已复制 `templates/dirty-worktree-ledger.md` 到 `changes/<change-id>/dirty-worktree-ledger.md`，并运行 `scripts/business-dirty-worktree-gate.sh <repo> --ledger changes/<change-id>/dirty-worktree-ledger.md`。
- [ ] 已按 `docs/skills-routing.md` 读取命中的 harness skill，并复制 `templates/skill-usage.md` 到 `changes/<change-id>/skill-usage.md`。
- [ ] `scripts/skill-usage-gate.sh changes/<change-id>` 已通过；命中场景未使用 skill 时已写 N/A reason。
...
- [ ] 如启用并行开发，控制面文件只允许 Orchestrator 写入。
...
- [ ] 用户确认本计划后，Orchestrator 默认自动执行到人工验收点、阻塞点或风险决策点；不得在机械步骤完成后停下询问是否继续。
...
- [ ] 已计划合并前临时写死扫描：`scripts/temp-hardcode-scan.sh <changed-files...>`。
- [ ] 已按 `docs/architecture/changes-retention-policy.md` 规划过程产物位置：Git 只保留轻量 Markdown 摘要，大文件放 `artifacts/<change-id>/` 或外部存储。

## 自动执行边界

用户确认 `spec.md`、`docs/contracts/<change-id>-api.md`、`technical-solution.md` 和本计划后，Orchestrator 持续执行下面的 checklist，直到到达人工验收点、阻塞点或风险决策点。
...
6. Verification Map：复制 `templates/verification-map.md` 到 `changes/<change-id>/verification-map.md`，把“什么叫做对”的关键约束逐条映射到命令、PC smoke、SQL 只读查询、人工确认或 `N/A:` 原因；运行 `scripts/verification-map-gate.sh changes/<change-id>`。
...
9. Agent Dispatch Plan：需要派发 Explorer / Reviewer / Test / Backend / Frontend / Mobile Agent 时，先运行 `scripts/agent-dispatch-plan.sh` 生成 `changes/<change-id>/agent-dispatch-plan.md`，再运行 `scripts/agent-dispatch-plan-gate.sh changes/<change-id>`；启用 Backend / Frontend / Mobile candidate implementation agent 前，先复制 `templates/agent-candidate-confirmation.md` 到 `changes/<change-id>/agent-candidate-confirmation.md`，确认 `candidate_dispatch_confirmation: CONFIRMED`、`business_code_start_gate: PASS`、`allowed_paths_confirmed: yes`，并保持 `protected_actions_allowed: no`、`global_config_write_allowed: no`、`db_or_release_actions_allowed: no`；Reviewer / Test Agent 输出后运行 `scripts/agent-output-contract-gate.sh changes/<change-id> <agent-id>`。
10. Business code-start：业务仓第一次改代码前运行 `scripts/business-code-start-gate.sh changes/<change-id> <changed-files...>`，确认分支不是 `main/master` 且路径已获准。
...
19. 临时写死扫描：运行 `scripts/temp-hardcode-scan.sh <changed-files...>`；命中 `CODX` / `smoke` / `mock` / `localhost` / `token` / `password` / `TODO` / `FIXME` 时先移除、配置化或记录已审 false positive。
...
22. 预发前确认：运行 `scripts/ai-test-report-gate.sh changes/<change-id>`；只有 `confirmation_status: CONFIRMED` 且 `recommendation: 允许进入预发` 才允许进入测试 / 预发发布。
...
25. Reviewer Agent：只读审查 spec、plan、contract、contract-delta、technical-solution、diff、evidence、backend-test-plan、pc-e2e-smoke-plan、pc-e2e-smoke-report、test-agent-verification、ai-test-report 和 harness-status，并按 Java checklist 复核后端改动；输出 `review.md` 后运行 `scripts/agent-output-contract-gate.sh changes/<change-id> sfa-harness-reviewer` 和 `scripts/reviewer-gate.sh changes/<change-id>`。
...
| --- | --- | --- | --- |
| Backend Agent | 后端独立 worktree 内的 Java、Mapper、后端测试；读取 change package，不在业务仓新建 OpenSpec | 只允许提出 contract delta；由 Orchestrator 合并控制面 contract、plan 和 verification map | Maven/targeted test 证据；如需改契约或任务，输出 delta 建议 |
| Frontend Agent | 前端独立 worktree 内的 Vue 页面、路由、前端 API 调用、前端 mock | 只读 contract；只按 delta 适配 | 窄范围 ESLint/build 证据；前端字段映射说明 |
| Orchestrator | 控制面 plan/evidence/review 协调、最终集成 | 唯一可写控制面；合并契约变更并通知前端 Agent | worktree gate、allowed paths、confidence gate、review |

Worktree 隔离规则：

1. Backend Agent 不直接写主业务仓，必须写后端独立 worktree。
2. Frontend Agent 不直接写主业务仓，必须写前端独立 worktree。
3. 实现 Agent 不得修改控制面文件；控制面只由 Orchestrator 更新。
4. Backend Agent 必须读取 Java 规范门禁文档并运行 `java-mechanical-quality.sh`。
5. Orchestrator 集成后必须重新运行 `java-mechanical-quality.sh`。
6. 即使后端、H5、iOS、Android 是不同 Git 仓，也默认给实现 Agent 使用独立 worktree；原因是保护主业务工作树、保留独立分支和 diff，方便 Orchestrator 独立集成或回退。
7. Orchestrator 在派发实现前运行：
...
1. Orchestrator 派发实现 Agent 后必须保持执行，不得只输出“Agent 已派发/等待返回”后停止。
2. Orchestrator 必须持续等待，直到所有 Agent 返回 `completed`、返回 blocker/risk，或用户明确要求暂停。
3. Agent 返回后，Orchestrator 负责读取各 worktree diff、同步 contract delta、更新 evidence/status、运行集成验证，再进入下一 gate。
4. 只有遇到跨 Agent 契约冲突、环境阻塞、真实环境写入、UI 人工确认或高风险决策时，才停止等待用户。

契约变更同步规则：

1. Backend Agent 需要改 contract 时，先停止实现并输出 delta 建议，不直接改控制面 contract。
2. Orchestrator 合并 contract 后，必须追加 `changes/<change-id>/contract-delta.md`。
3. Orchestrator 必须把 delta 摘要发给 Frontend Agent。
4. Frontend Agent 继续开发前必须读取最新 contract 和 contract-delta。
5. Reviewer Agent 必须检查“后端契约变更是否已被前端消费”。
6. Orchestrator 必须运行 `scripts/contract-delta-gate.sh <changed-files...>`，并把 `NOTICE` 输出作为前端通知内容。

## 计划修改路径

```yaml
frontend:
  - $SFA_REPO_FRONTEND_MAP_SYSTEM/src/**
backend:
  - $SFA_REPO_BACKEND_SALES_MANAGEMENT/**
control_plane:
  - $SFA_HARNESS_ROOT/changes/<change-id>/**
  - $SFA_HARNESS_ROOT/docs/contracts/<change-id>-api.md
```

## 验证命令

```bash
scripts/confidence-gate.sh changes/<change-id>/spec.md
scripts/technical-solution-gate.sh changes/<change-id>
scripts/verification-map-gate.sh changes/<change-id>
scripts/assumption-leak-gate.sh changes/<change-id>/spec.md <changed-files...>
scripts/workstream-dispatch-gate.sh changes/<change-id>
scripts/business-code-start-gate.sh changes/<change-id> <changed-files...>
scripts/parallel-worktree-gate.sh /path/to/backend-worktree /path/to/frontend-worktree
scripts/contract-delta-gate.sh <changed-files...>
scripts/allowed-paths.sh changes/<change-id>/spec.md <changed-files...>
scripts/java-mechanical-quality.sh "$SFA_REPO_BACKEND_SALES_MANAGEMENT" <changed-java-or-xml-files...>
scripts/canonical-command-gate.sh -- mvn -pl sfa-sales-management-interfaces -am -DskipTests compile
scripts/mvn-targeted-test.sh "$SFA_REPO_BACKEND_SALES_MANAGEMENT" <module> compile
scripts/frontend-lint-build.sh "$SFA_REPO_FRONTEND_MAP_SYSTEM" lint-files src/views/<feature>/*.vue
...
SFA_HARNESS_SMOKE=1 SFA_HARNESS_PROXY_LOG=artifacts/<change-id>/pc-e2e-smoke/local-proxy.ndjson node scripts/harness-local-proxy.mjs changes/<change-id>/local-routing.yml
...
| 两个 Agent 并行导致字段漂移 | Backend Agent 只提 delta，Orchestrator 合并 contract，Frontend Agent 通过 contract-delta 适配 |
| 多 Agent 共写工作树导致冲突 | 并行实现必须用独立 worktree；Orchestrator 最终集成 |
