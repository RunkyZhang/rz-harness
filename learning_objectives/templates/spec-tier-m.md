...
frontend_repo: $SFA_REPO_FRONTEND_MAP_SYSTEM
backend_repo: $SFA_REPO_BACKEND_SALES_MANAGEMENT
```
...
| Item | Value |
| --- | --- |
| Contract doc | `docs/contracts/<change-id>-api.md` |
...
这些文件不是默认强制产物；需要时必须在 `verification-map.md` 中映射到验证命令、人工确认、Test Agent 结论或 Reviewer 结论。业务仓已有 `openspec/` 仅作为 legacy context 读取，不作为新需求门禁来源。
...
allowed_paths:
  ...
  backend:
    - $SFA_REPO_BACKEND_SALES_MANAGEMENT/sfa-sales-management-interfaces/src/**
    - $SFA_REPO_BACKEND_SALES_MANAGEMENT/sfa-sales-management-application/src/**
    - $SFA_REPO_BACKEND_SALES_MANAGEMENT/sfa-sales-management-infrastructure/src/**
forbidden_paths:
