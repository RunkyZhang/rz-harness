...
- 这些文件不是默认强制产物；需要时必须在 `verification-map.md` 中映射到验证命令、人工确认、Test Agent 结论或 Reviewer 结论。
...
- 必须运行 `scripts/verification-map-gate.sh changes/<change-id>`，不得让 `TODO` / `TBD` / `BLOCKED` 行进入实现。
