# 测试 Agent 独立验收：<change-id>

> 由独立测试 Agent 维护，用于验收主 Agent 的实现、常规测试和证据。
> 主 Agent 负责修复、补常规测试和补证据；最终目标是否达成，只能由测试 Agent
...
- `PENDING`：测试 Agent 尚未开始验收。
- `ISSUES_FOUND`：已发现问题并回传主 Agent。
- `RETESTING`：主 Agent 已修复，等待或正在复测。
- `GOAL_ACHIEVED`：测试 Agent 确认开发目标达成。
- `BLOCKED`：测试 Agent 无法继续验收。
