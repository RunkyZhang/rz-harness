...
usage() {
  cat >&2 <<'USAGE'
Usage:
  scripts/change-scaffold.sh --tier S|M|L [--changes-dir <dir>] <change-id>
...
| --- | --- | --- | --- |
| Scaffold | \`scripts/change-scaffold.sh --tier $tier_upper $change_id\` | \`PASS\` | Initial artifact skeleton created. |
EOF
}
...
if [[ "$profile" == "tier-m" || "$profile" == "tier-l" ]]; then
  ...
  "$root/scripts/agent-dispatch-plan.sh" \
    --agent-id sfa-test-agent,sfa-harness-reviewer \
    --runtime codex_generated \
    --change-id "$change_id" \
    --output "$target/agent-dispatch-plan.md" >/dev/null
