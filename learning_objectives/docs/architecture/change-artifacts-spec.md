...
| `tier-m` | Tier M、普通全栈或多文件业务变更 | `spec.md`, `harness-status.md`, `evidence.md`, `plan.md`, `contract.md`, `technical-solution.md`, `verification-map.md`, `ai-test-plan.md`, `test-agent-verification.md`, `agent-dispatch-plan.md`, `skill-usage.md`, `review.md` | 把既有强制流程和 registry-derived Agent 派发计划一次 scaffold 出来，减少手工漏文件。`agent-candidate-confirmation.md` 只在启用 candidate implementation agent 时可选新增。 |
...
`scripts/change-artifacts-gate.sh` 读取该 marker。新 change 缺失 marker 时 fail-closed；历史 change 可用 `--historical` 只输出 warning。

## Commands

```bash
scripts/change-scaffold.sh --tier S <change-id>
scripts/change-artifacts-gate.sh changes/<change-id>
scripts/change-artifacts-gate.sh --historical changes/<old-change-id>
```
