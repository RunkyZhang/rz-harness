...
> 并行开发模式下使用。Backend Agent 只能提出 contract delta 建议；主 Agent 是唯一允许写控制面 contract / delta 的角色；Frontend Agent 只读取本文件并适配。
