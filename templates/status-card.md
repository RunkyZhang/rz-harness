...
> 用户可见的单一流程入口。只聚合状态，不替代 spec / contract / plan / technical-solution / evidence / report。
> 触发后主 Agent 先跑 `changes/status-card.sh changes/<change-id>`，再把摘要写入本文件。脚本不覆盖 Agent Roster、当前阻塞、人工确认待办。
...
| Verification run | `NOT_APPLICABLE / PENDING / PASS / FAIL / DRY_RUN / BLOCKED` | `changes/<change-id>/verification-run-report.md` | `verification-map.md` 有可执行行时，进入 Tester / Reviewer 前必须有运行报告 |
| Tester verification | `PENDING / ISSUES_FOUND / RETESTING / GOAL_ACHIEVED / BLOCKED` | `changes/<change-id>/test-agent-verification.md` | 未 `GOAL_ACHIEVED` 主 Agent 不得声明目标达成 |
...
> 每次派发、完成、阻塞、复测或因代码变更导致结果 stale 时都必须更新。`status-card.sh` 不算这张表。

| Agent Label | Runtime / Nickname | Role | Scope | Write Scope | Status | Output / Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `<change-id> / 主 Agent / control-plane` | `main-agent` | `主 Agent` | `流程编排、集成、必要修复、证据记录` | `changes/<change-id>/*` 和明确允许的业务文件 | `ACTIVE / BLOCKED / DONE` | `status-card.md / evidence.md` |
| `<change-id> / Explorer / pre-implementation-facts` | `<runtime nickname or N/A>` | `Explorer` | `方案或实现前只读查证` | `READ_ONLY` | `PENDING / RUNNING / DONE / BLOCKED / STALE` | `evidence.md / spec 供料` |
| `<change-id> / Test Strategy / ai-test-plan` | `<runtime nickname or N/A>` | `Test Strategy` | `方案确认后写 AI 测试方案` | `changes/<change-id>/ai-test-plan.md` | `PENDING / RUNNING / DONE / BLOCKED / STALE` | `ai-test-plan.md` |
| `<change-id> / Backend / <scope>` | `<runtime nickname or N/A>` | `Backend` | `<controller/service/mapper/test>` | `<repo/path allowlist or isolated worktree>` | `PENDING / RUNNING / DONE / BLOCKED / STALE` | `<report / test command / changed files>` |
| `<change-id> / Frontend / <scope>` | `<runtime nickname or N/A>` | `Frontend` | `<page/api/router/smoke>` | `<repo/path allowlist or isolated worktree>` | `PENDING / RUNNING / DONE / BLOCKED / STALE` | `<report / lint / screenshot>` |
| `<change-id> / Tester / ai-test-plan` | `<runtime nickname or N/A>` | `Tester` | `按 confirmed ai-test-plan 独立验收` | `changes/<change-id>/test-agent-verification.md only` | `PENDING / RUNNING / ISSUES_FOUND / GOAL_ACHIEVED / BLOCKED / STALE` | `test-agent-verification.md` |
| `<change-id> / Reviewer / final-readonly-review` | `<runtime nickname or N/A>` | `Reviewer` | `只读审查方案一致性、harness 约束、代码质量、测试证据` | `changes/<change-id>/review.md only` | `PENDING / RUNNING / PASS / BLOCKED / STALE` | `review.md / reviewer-gate` |
...
| Contract | `changes/<change-id>/contract.md` | `<status>` |
...
| Tester verification | `changes/<change-id>/test-agent-verification.md` | `<status>` |
