...
每个目标仓第一次修改业务代码之前，从 `main`/`master` 创建/切换到 `codex/<change-id>`，除非用户另有要求；记录基线分支+commit，永不在 `main`/`master` 上修改，并对计划文件通过 `scripts/business-code-start-gate.sh`。
...
- 若 Reviewer 产出之后又发生任何业务代码变更，将先前审查标为过期，按需重跑 Tester，然后重跑 Reviewer 和 `scripts/reviewer-gate.sh`。
...
`changes/<change-id>/skill-usage.md` 并运行 `scripts/skill-usage-gate.sh`。
...
`scripts/swagger-model-documentation-gate.sh <change-dir> <changed-files...>`；
该配置不追溯既往，不适用于仅用于导出的模型或基础设施 DTO。

前端/UI 变更：先判断复杂度。PRD UI / 交互编码需要 `ui-rule-checklist.md`、PRD UI 来源、匹配的规则，以及 `scripts/ui-rule-gate.sh changes/<change-id>`。
...
在真实 E2E 或依赖环境的测试之前，填写 `environment-readiness.md` 并运行 `scripts/environment-readiness-gate.sh changes/<change-id>`；记录环境、所需系统、本地运行标准、集成拓扑、角色/账号来源、数据、DB/写入边界、回滚、设备/工具、阻塞项，以及无可复用凭据。
...
档位 M/L 的方案文档在业务代码之前需要已确认的 `templates/technical-solution.md` 和 `scripts/technical-solution-gate.sh changes/<change-id>`。
...
方案确认之后，独立的 Test Strategy 创建 `ai-test-plan.md`；在实现之前需要用户确认以及 `scripts/ai-test-plan-gate.sh changes/<change-id>`。
...
在测试/预发发布之前，需要 `ai-test-report.md` 以及 `scripts/ai-test-report-gate.sh changes/<change-id>`；该关卡还要求已确认的测试方案和 Tester `GOAL_ACHIEVED`。

实现和验证之后，只读 Reviewer 必须在人工 review 或 PR 之前撰写 `changes/<change-id>/review.md` 并运行 `scripts/reviewer-gate.sh changes/<change-id>`。
...
合并前，对变更的业务文件运行 `scripts/diff-hygiene-gate.sh <repo> [--base <ref>] <files...>` 和 `scripts/temp-hardcode-scan.sh <files...>`。
...
对已执行的 E2E，环境就绪是清楚的；Tester 已确认 `GOAL_ACHIEVED` 或记录了 `BLOCKED`；Reviewer 产出已通过 `scripts/reviewer-gate.sh changes/<change-id>`。
残余风险和回滚已记录。
