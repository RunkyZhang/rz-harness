...
function logLine(config, event) {
  ...
  console.log(line);
  if (process.env.RZ_HARNESS_PROXY_LOG) {
    fs.appendFileSync(process.env.RZ_HARNESS_PROXY_LOG, `${line}\n`);
  }
}
...
function startProxy(config) {
  const host = process.env.RZ_HARNESS_PROXY_HOST || config.proxy_listen.host || '127.0.0.1';
  const port = Number(process.env.RZ_HARNESS_PROXY_PORT || config.proxy_listen.port || 19080);
