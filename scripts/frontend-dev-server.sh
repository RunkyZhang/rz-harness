...
target="${1:-}"
port="${2:-${RZ_FRONTEND_DEV_PORT:-9527}}"
host="${RZ_FRONTEND_DEV_HOST:-0.0.0.0}"

[[ -n "$target" ]] || { usage; fail "missing frontend repo id or path"; }

case "$target" in
  frontend-map-system|mapSystem)
    repo="${RZ_REPO_MAP_SYSTEM:-}"
    expected_node="${RZ_FRONTEND_NODE_VERSION:-14.21.3}"
    ;;
  frontend-sign-up|sign-up)
    repo="${RZ_REPO_SIGN_UP:-}"
    expected_node="${RZ_FRONTEND_SIGN_UP_NODE_VERSION:-${RZ_FRONTEND_NODE_VERSION:-14.21.3}}"
    ;;
  *)
    repo="$target"
    expected_node="${RZ_FRONTEND_NODE_VERSION:-14.21.3}"
...
[[ -n "$repo" ]] || fail "repo path is empty; check config/runtime_local.sh"
[[ -d "$repo" ]] || fail "repo path not found: $repo"
[[ -f "$repo/package.json" ]] || fail "package.json not found: $repo"

node_bin_dir="${RZ_FRONTEND_NODE_BIN_DIR:-}"
...
[[ -n "$actual_node" ]] || fail "node is not available; install Node $expected_node or set RZ_FRONTEND_NODE_BIN_DIR"

if [[ "$target" == "frontend-map-system" || "$target" == "mapSystem" || "$repo" == *"/mapSystem" ]]; then
  if [[ "$actual_node" != "$expected_node" ]]; then
    fail "mapSystem local dev expects Node $expected_node, current Node is $actual_node; set RZ_FRONTEND_NODE_BIN_DIR or install via nvm"
  fi
fi
