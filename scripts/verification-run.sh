...
gates/verification-map-gate.sh.
...
[[ -f "$map_file" ]] || fail \
  ...
  "Create verification-map.md and run gates/verification-map-gate.sh first."

"$root/gates/verification-map-gate.sh" "$change_dir_abs" >/dev/null
...
record_telemetry() {
  ...
  if [[ -n "${RZ_HARNESS_TELEMETRY_DIR:-}" ]]; then
    telemetry_args=(--telemetry-dir "$RZ_HARNESS_TELEMETRY_DIR")
  fi
  local failure_arg=()
...
if [[ "$row_count" -eq 0 ]]; then
  fail \
    ...
    "Add at least one VM-* row and run gates/verification-map-gate.sh first."
fi
