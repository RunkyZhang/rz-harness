...
usage() {
  cat >&2 <<'USAGE'
Usage:
  gates/change-artifacts-gate.sh [--historical] <change-dir>
  ...
  case "$1" in
    ...
    --*)
      fail \
        ...
        "gates/change-artifacts-gate.sh --historical changes/old-change"
      ;;
    *)
      break
      ;;
...
[[ -d "$change_dir" ]] || fail \
  ...
  "gates/change-artifacts-gate.sh changes/example"
...
if [[ "${#required[@]}" -gt 0 ]]; then
  ...
  if [[ "${#missing[@]}" -gt 0 ]]; then
    fail \
      ...
      "changes/change-scaffold.sh --tier ${profile#tier-} <change-id>"
  fi
fi
