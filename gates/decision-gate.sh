...
usage() {
  cat <<'USAGE'
Usage:
  gates/decision-gate.sh <decisions.md | change-dir>
  gates/decision-gate.sh --notify <decisions.md | change-dir>
...
Modes:
  ...
  --notify    Render pending decisions into Lark-ready text (DRY-RUN only).
              This MVP NEVER sends. If RZ_DECISION_LARK_CHAT is set it prints
              the lark-cli command it WOULD run, without executing it.

Environment:
  RZ_DECISION_LARK_CHAT   Target Lark chat/open_id (set in config/runtime_local.sh).
...
if [[ "$mode" == "notify" ]]; then
  ...
  printf '%s\n' "$msg"
  printf -- '---\n'
  if [[ -n "${RZ_DECISION_LARK_CHAT:-}" ]]; then
    printf 'DRY-RUN: would send (NOT executed) -> lark-cli im send --chat %q --text <上述文案>\n' "$RZ_DECISION_LARK_CHAT"
  else
    printf 'NOTE: set RZ_DECISION_LARK_CHAT in config/runtime_local.sh to target a chat. This MVP never auto-sends; copy the text above or wire --send in a follow-up change.\n'
  fi
  exit 0
fi
