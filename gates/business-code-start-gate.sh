...
usage() {
  cat <<'USAGE'
Usage:
  gates/business-code-start-gate.sh <change-dir> <changed-file> [changed-file...]
...
Checks before editing business repositories:
  ...
  - changed files are listed under spec.md allowed_paths
...
[[ -n "$change_dir" ]] || fail \
  ...
  "gates/business-code-start-gate.sh changes/<change-id> /absolute/path/File.java"
[[ -d "$change_dir" ]] || fail \
...
"$root/gates/technical-solution-gate.sh" "$change_dir" >/dev/null
"$root/gates/verification-map-gate.sh" "$change_dir" >/dev/null
...
# workstream-dispatch-gate 检查多仓 / 全栈工作线拆分；RZ 未引入或单人串行时跳过
if [[ -f "$root/gates/workstream-dispatch-gate.sh" ]]; then
  "$root/gates/workstream-dispatch-gate.sh" "$change_dir" >/dev/null
fi

spec="$change_dir/spec.md"
[[ -f "$spec" ]] || fail \
  "BUSINESS_CODE_START/MISSING_SPEC" \
  "spec.md not found in $change_dir" \
  "Create spec.md with allowed_paths before editing business repos." \
...
[[ "${#allowed_rules[@]}" -gt 0 ]] || fail \
  ...
  "no absolute allowed_paths found in $spec" \
  "Add absolute business allowed_paths to spec.md before code start." \
  "allowed_paths:\n  - /absolute/repo/src/**"
...
for changed in "$@"; do
  ...
  [[ "$allowed" == "yes" ]] || fail \
    ...
    "changed file is not covered by spec allowed_paths: $file" \
    "Add the path to spec.md allowed_paths, or do not edit this file." \
    "$spec"
...
for repo_root in "${repo_roots[@]}"; do
  ...
  [[ -n "$branch" ]] || fail \
    ...
    "Switch to a harness/<change-id> branch before editing." \
    "git -C $repo_root switch -c harness/<change-id>"
  case "$branch" in
    main|master)
      fail \
        "BUSINESS_CODE_START/PROTECTED_BRANCH" \
        "business repo is on protected branch '$branch': $repo_root" \
        "Create or switch to a harness/<change-id> branch before editing business code." \
        "git -C $repo_root switch -c harness/<change-id>"
      ;;
  esac
done
