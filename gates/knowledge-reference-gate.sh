...
usage() {
  cat <<'USAGE'
Usage:
  gates/knowledge-reference-gate.sh <change-file> [change-file...]
  gates/knowledge-reference-gate.sh --audit
...
id_regex='(RZ-PIT-[0-9]+|RZ-SMP-[0-9]+|DEC-[0-9]{4}-[0-9]{2}-[0-9]{2}-[A-Za-z0-9-]+)'
...
active_entry_ids() {
  grep -rhE "^id:[[:space:]]*(RZ-PIT-[0-9]+|RZ-SMP-[0-9]+)[[:space:]]*$" \
